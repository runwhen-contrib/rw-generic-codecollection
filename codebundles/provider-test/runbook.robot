*** Settings ***
Documentation       Does the needful by testing provider secrets with a runbook
...
Test Secret Providers
    ...
    RW.Core.Add Pre To Report    Reporting a score of: ${TOTAL_SCORE}

*** Keywords ***
Suite Initialization
    ${process}=    Run Process    ${CURDIR}/printenv.sh
    ...
    Log To Console    ${process.stdout}
    IF    ${process.rc} == 0
        ${TOTAL_SCORE}=    Evaluate    ${TOTAL_SCORE} + 0.5
    END
    ...
    ${jvaultval}=    Set Variable    ${jvault-simple}
    Log To Console    ${kubeconfig.value}
    Log To Console    ${jvaultval.value}
    ...
    Set Suite Variable   ${TOTAL_SCORE}    ${TOTAL_SCORE}
